...
from .forms import NameForm, Role, EditProfileForm, EditProfileAdminForm, PostForm, CommentForm, EditAvatarForm, DeletePostForm
from flask import redirect, render_template, session, url_for, flash, request, make_response
...
def edit_avatar():
    ...
    if form.validate_on_submit():
        ...
        if not flag:
            flash('文件类型错误')
            return redirect(url_for('.user', username=current_user.fname))
        avatar.save('{}{}_{}'.format(UPLOAD_FOLDER, current_user.username, fname))
        current_user.rel_avatar = '/static/images/{}_{}'.format(current_user.username, fname)
        db.session.add(current_user)
        flash('头像已更换')
        return redirect(url_for('.user', username=current_user.username))
